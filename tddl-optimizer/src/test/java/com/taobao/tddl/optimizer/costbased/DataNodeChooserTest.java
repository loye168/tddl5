...
public class DataNodeChooserTest extends BaseOptimizerTest {
    ...
    public void test_join查询_生成JoinMergeJoin_一张广播表_一张单库表_开启优化参数() {
        KVIndexNode table1 = new KVIndexNode("TABLE7");// 广播表
        QueryNode query = new QueryNode(table1);

        KVIndexNode table2 = new KVIndexNode("TABLE8");// 一张单表
        KVIndexNode table3 = new KVIndexNode("TABLE1");// 分库表
        JoinNode table4 = table2.join(table3, "ID", "ID");

        JoinNode join = query.join(table4, "ID", "TABLE1.ID");
        join.build();
        QueryTreeNode qtn = shard(join, false, true);
        Assert.assertTrue(qtn instanceof JoinNode);
    }
    ...
    public void test_insertSelect查询_select为join_query查询_生成merge_设置where条件() {
        ...
        KVIndexNode table3 = new KVIndexNode("TABLE3");
        table3.keyQuery("ID IN (2,3)");
        QueryNode query = new QueryNode(table3);
        JoinNode join = table2.join(query, "ID", "ID");
        join.select("TABLE2.*");
    ...
    public void test_insertSelect查询_select广播表_生成merge_设置where条件() {
        KVIndexNode table1 = new KVIndexNode("TABLE1");
        KVIndexNode table2 = new KVIndexNode("TABLE7");// 广播表
        table2.keyQuery("ID IN (2,3)");
        InsertNode insert = new InsertNode(table1);
        insert.setSelectNode(table2);
        insert.build();
    ...
    public void test_insertSelect查询_都是广播表_生成merge_设置where条件() {
        KVIndexNode table1 = new KVIndexNode("TABLE7");
        KVIndexNode table2 = new KVIndexNode("TABLE7");// 广播表
        table2.keyQuery("ID IN (2,3)");
        InsertNode insert = new InsertNode(table1);
        insert.setSelectNode(table2);
    ...
    public void test_insertSelect查询_select不同joinGroup_生成merge_设置where条件() {
        KVIndexNode table1 = new KVIndexNode("TABLE1");
        KVIndexNode table2 = new KVIndexNode("TABLE5");// 广播表
        InsertNode insert = new InsertNode(table1);
        insert.setSelectNode(table2);
        insert.build();
        try {
    ...
    public void test_insertSelect查询_other的joinGroup_生成merge_设置where条件() {
        KVIndexNode table1 = new KVIndexNode("TABLE5");
        KVIndexNode table2 = new KVIndexNode("TABLE6");
        table2.keyQuery("ID IN (2,3)");
        InsertNode insert = new InsertNode(table1);
        insert.setSelectNode(table2);
        insert.build();
    ...
    public void test_insertSelect查询_一张广播表_一张单表_生成merge_设置where条件() {
        KVIndexNode table1 = new KVIndexNode("TABLE7");
        KVIndexNode table2 = new KVIndexNode("TABLE8");
        table2.keyQuery("ID IN (2,3)");
        InsertNode insert = new InsertNode(table1);
        insert.setSelectNode(table2);
        insert.build();
        ASTNode qtn = shard(insert);
        Assert.assertTrue(qtn instanceof InsertNode);
    }

    @Test
    public void test_insertSelect查询_都是单表_生成merge_设置where条件() {
        KVIndexNode table1 = new KVIndexNode("TABLE8");
        KVIndexNode table2 = new KVIndexNode("TABLE8");
        table2.keyQuery("ID IN (2,3)");
        InsertNode insert = new InsertNode(table1);
        insert.setSelectNode(table2);
