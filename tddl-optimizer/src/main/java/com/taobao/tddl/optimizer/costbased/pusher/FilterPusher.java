...
public class FilterPusher {
    ...
    private static List<IFilter> copyFilterToJoinOnColumns(List<IFilter> DNF, List<ISelectable> qnColumns,
        ...
        for (IFilter bool : DNF) {
            if (bool instanceof IBooleanFilter) {
                int index = qnColumns.indexOf(((IBooleanFilter) bool).getColumn());
                if (index >= 0) {// 只考虑在源查找，在目标查找在上一层进行控制
                    IBooleanFilter o = ASTNodeFactory.getInstance()
                        .createBooleanFilter()
                        .setOperation(bool.getOperation());
                    o.setColumn(otherColumns.get(index).copy());
            ...
            } else if (bool instanceof IGroupFilter) {
                int index = qnColumns.indexOf(((IGroupFilter) bool).getColumn());
                if (index >= 0) {// 只考虑在源查找，在目标查找在上一层进行控制
                    IGroupFilter gf = (IGroupFilter) bool.copy();
                    // 更新所有子节点
                    gf.setColumn(otherColumns.get(index).copy());
                    for (IFilter sf : ((IGroupFilter) gf).getSubFilter()) {
                        ((IBooleanFilter) sf).setColumn(otherColumns.get(index).copy());
                    }
                    newIFilterToPush.add(gf);
                }
            }
        }
    ...
    private static boolean tryPushColumn(IFilter filter, boolean isColumn, QueryTreeNode qtn, boolean outer) {
        ...
        if (filter instanceof IGroupFilter) {
            if (isColumn) {
                value = ((IGroupFilter) filter).getColumn();
                if (value instanceof ISelectable) {
                    ISelectable c = qtn.findColumn((ISelectable) value);
                    if (c instanceof IColumn) {
                        if (!outer) {
                            // 更新所有子节点
                            ((IGroupFilter) filter).setColumn(c.copy());
                            for (IFilter sf : ((IGroupFilter) filter).getSubFilter()) {
                                ((IBooleanFilter) sf).setColumn(c.copy());
                            }
                        }
                        return true;
                    } else {
