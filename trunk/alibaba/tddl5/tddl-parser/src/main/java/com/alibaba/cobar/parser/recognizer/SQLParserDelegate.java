...
import com.alibaba.cobar.parser.recognizer.mysql.syntax.ReloadParser;
...
public final class SQLParserDelegate {
    ...
    private static enum SpecialIdentifier {
        ROLLBACK, SAVEPOINT, TRUNCATE, RELOAD
    }
        ...
        try {
            ...
            if (isEOF) {
                ...
                if (lexer.token() != MySQLToken.EOF) {
                    throw new SQLSyntaxErrorException(buildErrorMsg(null, lexer, sql));
                }
            }
            return stmt;
        } catch (Exception e) {
