...
public class SqlSelectSingleOptimizerTest extends BaseSqlOptimizerTest {
    ...
    public void testInsertIntoSelect() throws Exception {
        {
            SqlMergeNode node = getMergeNode("insert into student select * from student");
            System.out.println(node);
            Assert.assertEquals("insert into student ( ID, NAME, SCHOOL) select STUDENT.ID,STUDENT.NAME,STUDENT.SCHOOL from student",
                getSql0(node));
        }

        {
            SqlMergeNode node = getMergeNode("insert into student(id,name) select a.id,b.name from student a join (select * from student) b on a.name = b.name");
            System.out.println(node);
            Assert.assertEquals("insert into student ( ID, NAME) select A.ID,B.NAME from student A join  ( select student.ID,student.NAME,student.SCHOOL from student )  B  on A.NAME = B.NAME",
                getSql0(node));
        }
