...
public class TableMetaParser {
    ...
    private ColumnMeta parseColumn(String tableName, Node node) {
        ...
        if (autoIncrementNode != null) {
            autoIncrement = BooleanUtils.toBoolean(autoIncrementNode.getNodeValue());
        }

        return new ColumnMeta(tableName, name, getDataType(type), alias, nullable, autoIncrement);
    }
